...
export function readOpencodeOnlyFiles({root = REPOSITORY_ROOT} = {}) {
  const specializationPath = repositoryPath(root, 'registry', 'manifests', 'specialization.yaml');
  const projectConfigPath = repositoryPath(root, 'opencode.json');
  const agentsPath = repositoryPath(root, '.opencode', 'agents');
  const agents = new Map();

  if (fs.existsSync(agentsPath)) {
    for (const name of fs.readdirSync(agentsPath)) {
      if (name.endsWith('.md')) agents.set(path.basename(name, '.md'), readAgent(repositoryPath(root, '.opencode', 'agents', name)));
    }
  }

  return {
    specialization: fs.existsSync(specializationPath) ? readJson(specializationPath) : null,
    projectConfig: fs.existsSync(projectConfigPath) ? readJson(projectConfigPath) : null,
    agents,
  };
}
