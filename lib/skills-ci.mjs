...
export function staticIntegrity() {
  ...
  if (cursorMarket.status !== 'PASS') errors.push(...cursorMarket.errors.map(error => `cursor-marketplace: ${error}`));
  const result = { generated_at: now(), status: errors.length ? 'FAIL_STATIC_INTEGRITY' : 'PASS', errors, counts: { packages: packages.length, capabilities: capabilities.length, exposures: exposures.length, cursor_plugins: cursorMarket.plugins.length }, source: 'packages.lock.yaml + capabilities.yaml + exposures.yaml + cursor marketplace' };
