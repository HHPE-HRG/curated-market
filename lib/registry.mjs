...
export function validate(){
  ...
  for(const c of capabilities){
    ...
    const src=path.join(packageRoot(p),c.source_path);
    if(!exists(src)) errors.push(`missing source ${c.capability_id}: ${c.source_path}`);
    if(c.type==='skill'&&!exists(path.join(src,'SKILL.md'))) errors.push(`missing SKILL.md ${c.capability_id}`);
    for(const rel of c.requires?.files||[]) if(!exists(path.join(packageRoot(p),rel))) errors.push(`missing dependency ${c.capability_id}: ${rel}`);
  }
  ...
  const toolManifest=read('tools.yaml');for(const t of toolManifest.tools||[])for(const b of t.binary_paths||[]){if(!exists(b))errors.push(`missing tool binary ${t.tool_id}: ${b}`);else{const v=run(b,['--version']);if(v.status||!(v.stdout+v.stderr).includes(t.version))errors.push(`tool version mismatch ${t.tool_id}: ${b}`);}}
  const state=read('migration-state.yaml');
  const allowedToolTargets=new Set((toolManifest.tools||[]).flatMap(t=>t.source_binary_paths||[]).filter(exists).map(real));
  for(const o of state.managed_objects||[]) if(o.classification==='created_by_hhpe'&&o.kind==='symlink'&&exists(o.path)){
    if(!fs.lstatSync(o.path).isSymbolicLink()) errors.push(`managed link replaced ${o.path}`);else if(!real(o.path).startsWith(ROOT+path.sep)&&!allowedToolTargets.has(real(o.path)))errors.push(`managed link escapes registry ${o.path}`);
  }
  const result={generated_at:now(),status:errors.length?'failed':'passed',errors,warnings,counts:{packages:packages.length,capabilities:capabilities.length,exposures:exposures.length}};
...
export function status(){const v=validate();const s=read('migration-state.yaml');return {root:ROOT,validation:v.status,phase:s.phase,managed_objects:s.managed_objects.length,limitations:s.limitations};}
...
export function update(args=[]){
  ...
  const selected=args.includes('--all')?read('packages.lock.yaml').packages.map(p=>p.package_id):[args[args.indexOf('--package')+1]].filter(Boolean);
  if(!selected.length) return {mode:'check',message:'use --package <id> or --all; activation is never from a moving branch'};
  return {mode:'check',packages:selected.map(id=>{const p=read('packages.lock.yaml').packages.find(x=>x.package_id===id);if(!p)return {package:id,error:'unknown'};const root=packageRoot(p);const f=run('git',['-C',root,'fetch','--dry-run','origin'],{timeout:15000});return {package:id,pinned:p.revision.value,fetch_status:f.status,output:(f.stdout+f.stderr).trim()};})};
}
