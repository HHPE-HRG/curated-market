...
import {validateCursorMarketplace} from './cursor-marketplace.mjs';
...
export function validate(){
  ...
  const cursorMarket=validateCursorMarketplace();
  if(cursorMarket.status!=='PASS') for(const err of cursorMarket.errors) errors.push(`cursor-marketplace: ${err}`);
  const result={generated_at:now(),status:errors.length?'failed':'passed',errors,warnings,counts:{packages:packages.length,capabilities:capabilities.length,exposures:exposures.length,cursor_plugins:cursorMarket.plugins.length},policy:{lifecycle_owner:finalStack?.lifecycle_owner??null,superpowers_bootstrap:finalStack?.superpowers?.bootstrap??null},cursor_marketplace:cursorMarket.status};
