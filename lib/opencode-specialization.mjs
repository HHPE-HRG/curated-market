...
const parseAgentFrontmatter = source => {
  ...
  for (const line of source.split('\n')) {
    ...
    if (nested && permission) {
      if (Object.hasOwn(permission, nested[1])) return null;
...
const childPath = (base, key, parent) => Array.isArray(parent)
  ? `${base}[${key}]`
  : `${base}${base === 'opencode.json' ? ' ' : '.'}${key}`;

const firstDifferencePath = (actual, expected, location) => {
  if (sameJson(actual, expected)) return null;
  if (!actual || !expected || typeof actual !== 'object' || typeof expected !== 'object'
    || Array.isArray(actual) !== Array.isArray(expected)) return location;

  const actualKeys = Object.keys(actual);
  const expectedKeys = Object.keys(expected);
  for (const key of expectedKeys) {
    if (!Object.hasOwn(actual, key)) return childPath(location, key, expected);
  }
  for (const key of actualKeys) {
    if (!Object.hasOwn(expected, key)) return childPath(location, key, actual);
  }
  for (const key of expectedKeys) {
    const difference = firstDifferencePath(actual[key], expected[key], childPath(location, key, expected));
    if (difference) return difference;
  }
  return location;
};

const unsafeConfigString = value =>
  /OPENAI_API_KEY|api[_-]?key|auth\.json|credentials?|password|secret|token|opencode2|node_modules|\blatest\b/i.test(value)
  || /^(?:\.{1,2}[\\/]|~[\\/]|[\\/]|[A-Za-z]:[\\/]|file:)/i.test(value)
  || /(?:git(?:\+https?|ssh)?:\/\/|git@|https?:\/\/(?:www\.)?(?:github|gitlab|bitbucket)\.|github:|#[A-Za-z][\w./-]*$)/i.test(value);

const forbiddenConfigPaths = (value, location = 'opencode.json', paths = new Set()) => {
  if (typeof value === 'string') {
    if (unsafeConfigString(value)) paths.add(location);
    return paths;
  }
  if (!value || typeof value !== 'object') return paths;
  for (const [key, child] of Object.entries(value)) {
    const locationForChild = childPath(location, key, value);
    if (/credential|api[_-]?key|password|secret|token|auth|branch|git|accounts?|models?|^env(?:ironment)?$/i.test(key)) {
      paths.add(locationForChild);
    } else {
      forbiddenConfigPaths(child, locationForChild, paths);
    }
  }
  return paths;
};
...
export function validateOpencodeOnly({root, specialization, projectConfig, agents} = {}) {
  ...
  const policyDifference = firstDifferencePath(policy, APPROVED_OPENCODE_ONLY_POLICY, 'specialization');
  if (policyDifference) errors.push(`${policyDifference} does not match approved opencode_only policy`);
  if (!config || typeof config !== 'object' || Array.isArray(config)) {
    errors.push('opencode project configuration must be an object');
  } else {
    const providersDifference = firstDifferencePath(config.enabled_providers, ['openai', 'cursor'], 'opencode.json enabled_providers');
    if (providersDifference) errors.push(`${providersDifference} must enable only openai and cursor`);
    const pluginDifference = firstDifferencePath(config.plugin, ['cursor-opencode-provider@0.6.3'], 'opencode.json plugin');
    if (pluginDifference) errors.push(`${pluginDifference} must pin cursor-opencode-provider@0.6.3`);
    const instructionsDifference = firstDifferencePath(config.instructions, ['AGENTS.md'], 'opencode.json instructions');
    if (instructionsDifference) errors.push(`${instructionsDifference} must use relative AGENTS.md instructions`);
    for (const key of Object.keys(config)) {
      if (!['$schema', 'enabled_providers', 'plugin', 'instructions'].includes(key)) errors.push(`opencode.json ${key} is unsupported`);
    }
    for (const location of forbiddenConfigPaths(config)) errors.push(`${location} is forbidden`);
  }
