...
- T3-bound OpenCode expects Cursor-style project metadata under `~/.cache/opencode/projects/<workspace-slug>/`. Run `node scripts/ensure-t3-opencode-bindings.mjs` after checkout or when Glob/Grep fail with `ripgrep execution failed` on `agent-transcripts`.
