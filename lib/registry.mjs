...
export function validate(){
  ...
  for(const p of packages){
    ...
    // overlay/companion packages are owned references, not commit-pinned upstream checkouts
    if(p.revision.type==='overlay'||p.revision.type==='companion'){
      if(p.license?.path&&!exists(path.join(root,p.license.path))) errors.push(`missing license ${p.package_id}`);
      continue;
    }
...
export function update(args=[]){
  ...
  return {mode:'check',packages:selected.map(id=>{const p=read('packages.lock.yaml').packages.find(x=>x.package_id===id);if(!p)return {package:id,error:'unknown'};if(p.revision.type==='overlay')return {package:id,pinned:p.revision.value,source:'local overlay'};if(p.revision.type==='companion')return {package:id,pinned:p.revision.value,source:'owned companion'};const f=run('git',['ls-remote','--heads','--tags',p.repository],{timeout:15000});return {package:id,pinned:p.revision.value,fetch_status:f.status,remote_refs:f.status?null:f.stdout.trim().split('\n').length,output:f.status?(f.stderr||f.stdout).trim():null};})};
}
