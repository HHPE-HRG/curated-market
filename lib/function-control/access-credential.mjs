...
} from '../../registry/providers/cursor/refresh.mjs';
import { singleflightRefresh } from './refresh-coordinator.mjs';

const REFRESH_SKEW_MS = 60_000;

/**
 * Dispatch refresh by account provider_family. OpenCode never sees refresh tokens.
 *
 * @param {object} deps
 */
export function createAccessCredentialService(deps) {
  const { vault, accountStore, refreshFn } = deps;
   ...
   */
  async function refreshForAccount(accountId, refreshToken, options) {
    if (typeof refreshFn === 'function') {
      return refreshFn(refreshToken, { ...options, accountId });
    }
    const account = accountStore.getAccount(accountId);
    const family = account?.provider_family || String(accountId).split(':')[0];
    if (family === 'cursor') {
      return refreshCursorAccess(refreshToken, options);
    }
    return refreshOpenAIAccess(refreshToken, options);
  }
   ...
   * @param {Error} err
   */
  function classifyForAccount(accountId, err) {
    const account = accountStore.getAccount(accountId);
    const family = account?.provider_family || String(accountId).split(':')[0];
    if (family === 'cursor') {
      return classifyCursorRefreshError(err);
    }
    ...
    async ensureAccessCredential(accountId, options = {}) {
      ...
      if (needsRefresh) {
        const refreshed = await singleflightRefresh(accountId, async () => {
          try {
            const result = await refreshForAccount(accountId, refreshToken, {
            ...
            const classified = classifyForAccount(accountId, err);
