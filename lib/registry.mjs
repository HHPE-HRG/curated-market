...
import {readToolSpecs, validateToolManifest} from './tool-contracts.mjs';
...
export function validate(){
  ...
  if(toolManifest.schema_version===1)for(const t of readToolSpecs(toolManifest))for(const b of t.binary_paths||[]){if(!exists(b))errors.push(`missing tool binary ${t.tool_id}: ${b}`);else{const v=run(b,['--version']);if(v.status||!(v.stdout+v.stderr).includes(t.version))errors.push(`tool version mismatch ${t.tool_id}: ${b}`);}}
  else errors.push(...validateToolManifest(toolManifest,ids));
