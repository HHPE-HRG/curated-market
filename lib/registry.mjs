...
export function validate(){
  ...
  for(const o of state.managed_objects||[]) if(o.classification==='created_by_hhpe'&&o.kind==='symlink'&&exists(o.path)){
    if(!fs.lstatSync(o.path).isSymbolicLink()) errors.push(`managed link replaced ${o.path}`);
    else { try { const target=real(o.path); if(o.source&&target!==real(o.source)) errors.push(`managed link retargeted ${o.path}`); if(!target.startsWith(ROOT+path.sep)&&!allowedToolTargets.has(target)) errors.push(`managed link escapes registry ${o.path}`); } catch { errors.push(`managed link is broken ${o.path}`); } }
  }
...
export function status(){const v=validate();const s=read('migration-state.yaml');const f=fs.existsSync(path.join(M,'final-stack.yaml'))?read('final-stack.yaml'):{};return {root:ROOT,validation:v.status,phase:s.phase,managed_objects:s.managed_objects.length,limitations:s.limitations,hhpe_runtime_binding:{root:ROOT,policy_manifest:fs.existsSync(path.join(M,'final-stack.yaml'))?'registry/manifests/final-stack.yaml':null},ce_lifecycle_owner:f.lifecycle_owner??null,superpowers_bootstrap:f.superpowers?.bootstrap??null,retained_superpowers:f.superpowers?.retained_support??[],inactive_superpowers:f.superpowers?.inactive??[],host_parity:'documented; per-host native discovery pending',container_parity:'bundle contract documented; execution pending',remote_parity:'bundle contract documented; execution pending',strict_wrapper_verification:'blocked by unrelated topology/application checks'};}
